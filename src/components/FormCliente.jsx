...
export default function FormCliente({ onGuardado, onCancelar, clienteEditar = null }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (esEdicion) {
        await clientesAPI.editar(form.id, form)
      } else {
      ...
      <div style={{
        ...
        <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid var(--border)",
          ...
          <div>
            ...
            <div style={{ fontSize: "12px", color: "var(--text3)", marginTop: "2px" }}>
              {esEdicion ? `Editando: ${clienteEditar.nombre}` : "Completa los datos del cliente"}
            </div>
          </div>
        ...
        <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid var(--border)",
          ...
          <button className="btn btn-primary" onClick={handleSubmit} disabled={loading}>
            {loading ? "Guardando..." : esEdicion ? "Guardar cambios" : "Guardar cliente"}
          </button>
        </div>
      </div>
    </div>
